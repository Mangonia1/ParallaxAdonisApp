...
const Tipocuestionario=use('App/Models/Tipocuestionario');
const Cuestionariomod=use('App/Models/Cuestionariomod');
...
// ============CUESTIONARIO FAKE=========================
    ...
    async vertipocuestionarios({request,response})
    {
    return await Database
    .select('tipocuestionarios.id as id',
    'tipocuestionarios.nombretipocuestionario as nombretipocuestionario',
    'tipocuestionarios.visible as visible'
    )
    .from('tipocuestionarios')
    .where( 'tipocuestionarios.visible', 1);
    }

    async guardartipocuestionario({request,response})
    {
        const quis = new Tipocuestionario()
        quis.nombretipocuestionario = request.input('nombretipocuestionario');
        quis.visible = '1';
        await quis.save();
        return await response.status(200).json(quis);
    }

    async editartipocuestionario({params,request,response})
    {
        const quis=await Tipocuestionario.find(params.id);
        quis.nombretipocuestionario = request.input('nombretipocuestionario');
        await quis.save();
        return await response.status(200).json(quis);
    }

    async eliminartipocuestionario({params,request,response})
    {
        const quis=await Tipocuestionario.find(params.id);
        quis.visible = '2';
        await quis.save();
    ...
    async vertipocuestionariouno({params,request,response})
    {
        const cuestionario=await Tipocuestionario.find(params.id);
...
async vercuestionariomodel({params,request,response})
    {
       return await Database
       .select('cuestionariomods.id as id',
       'cuestionariomods.idtipocuestionario as idtipocuestionario',
        'cuestionariomods.preguntatxt as preguntatxt',
        'cuestionariomods.tiporespuesta as tiporespuesta'
       )
       .from('cuestionariomods')
       .where('cuestionariomods.visible', 1)
       .where('cuestionariomods.idtipocuestionario', params.id);
    }

    async guardarcuestionariomodel({request,response})
    {
        const preg = new Cuestionariomod()
        preg.idtipocuestionario = request.input('idtipocuestionario');
    ...
    async editarcuestionariomodel({params,request,response})
    {
        const preg=await Cuestionariomod.find(params.id);
        preg.idtipocuestionario = request.input('idtipocuestionario');
        preg.preguntatxt = request.input('preguntatxt');
        preg.tiporespuesta = request.input('tiporespuesta');
        // preg.respondido = 'no';
    ...
    async eliminarcuestionariomodel({params,request,response})
    {
        const preg=await Cuestionariomod.find(params.id);
        preg.visible = '2';
        await preg.save();
    ...
    async vercuestionariomodeluno({params,request,response})
    {
        const pregunta=await Cuestionariomod.find(params.id);

        return response.status(200).json(pregunta);
    }

    async vercuestionariomoeldvisible({params,request,response})
    {
       return await Database
...
// ============CUESTIONARIOMODEL=========================
    ...
    async resultadocuestionario({params,request,response})
    {
        const quis=await Cuestionario.find(params.id);
        quis.resultado = request.input('resultado');
        await quis.save();
        return await response.status(200).json(quis);
    }
